// src/redux/middleware/databaseMiddleware.ts
...
export const databaseMiddleware: Middleware<{}, RootState, AppDispatch> = 
  store => next => action => {
    ...
    if (action.type === 'network/statusChanged' && action.payload.isConnected) {
    ...
    if (action.type.endsWith('/rejected') && action.error?.name === 'NetworkError') {
